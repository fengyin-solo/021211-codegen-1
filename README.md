...
- 缩放、适合宽度、页码导航
- 左侧页面缩略图栏：按当前外观缩小显示、当前页高亮、点击跳页、滚动联动
- 文件选择和拖拽上传
...
│       │   └── ThumbnailPanel.vue # 左侧页面缩略图栏（虚拟列表 + LRU）
│       ├── styles/
│       │   └── global.scss     # 全局样式
│       └── utils/
...
- 缩略图虚拟列表（DOM 中仅保留视口附近约 12 项）+ 位图 LRU（最多缓存 40 张），200 页不一次性绘制
- 逐页尺寸预计算，支持混合页面大小
- 渲染版本控制，缩放时取消过期任务
- rAF 滚动节流 + CSS GPU 加速
...
- 左侧固定一栏，每页按真实纸张比例缩小为 128px 宽缩略图，纵向排列
- 当前阅读页缩略图蓝色高亮；滚动正文时高亮与列表位置跟随变化
- 点击缩略图平滑跳转到对应页；缩略图列表与正文互不干扰各自滚动
- 边滚动边生成：只有进入缩略图视口（含缓冲区）的页面才会渲染，离开后位图进入 LRU，超出容量即回收
- 三种占位状态：等待生成 / 正在生成（转圈）/ 生成失败（显示原因并可重试），整栏不会出现空白
- 正文缩放后缩略图按新外观全部作废重生成；打开新文档时整栏按新文档重建
- 右上角箭头可收起为窄轨，再次点击展开
